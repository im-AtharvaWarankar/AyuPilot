...
class Patient(AtomicBaseModel):
    ...
    age = models.PositiveIntegerField(verbose_name=_('Age'), db_column='age')
    ...
    prakritiVata = models.PositiveIntegerField(verbose_name=_('Prakriti Vata %'), default=33, db_column='prakriti_vata')
    prakritiPitta = models.PositiveIntegerField(verbose_name=_('Prakriti Pitta %'), default=33, db_column='prakriti_pitta')
    prakritiKapha = models.PositiveIntegerField(verbose_name=_('Prakriti Kapha %'), default=34, db_column='prakriti_kapha')
    vikritiVata = models.PositiveIntegerField(verbose_name=_('Vikriti Vata %'), default=33, db_column='vikriti_vata')
    vikritiPitta = models.PositiveIntegerField(verbose_name=_('Vikriti Pitta %'), default=33, db_column='vikriti_pitta')
    vikritiKapha = models.PositiveIntegerField(verbose_name=_('Vikriti Kapha %'), default=34, db_column='vikriti_kapha')
    ...
    def clean(self):
        ...
        if self.prakritiVata + self.prakritiPitta + self.prakritiKapha != 100:
            raise ValidationError({'prakriti': 'Prakriti percentages must add up to 100.'})
        if self.vikritiVata + self.vikritiPitta + self.vikritiKapha != 100:
            raise ValidationError({'vikriti': 'Vikriti percentages must add up to 100.'})

    def __str__(self):
        return f"{self.name} - {self.age}y"


class Medication(AtomicBaseModel):
    """Current medications for patients"""
    
    patientId = models.ForeignKey(Patient, verbose_name=_('Patient'), related_name='medications', db_column='patient_id', on_delete=models.CASCADE)
    name = models.CharField(verbose_name=_('Medication Name'), max_length=255, db_column='name')
...
class Appointment(AtomicBaseModel):
    ...
    class Meta:
        ...
        managed = True
        indexes = [
            models.Index(fields=['appointmentDate', 'appointmentTime']),
            models.Index(fields=['doctorId', 'appointmentDate']),
            models.Index(fields=['status', 'appointmentDate']),
        ]
...
class ImageAnalysis(AtomicBaseModel):
    ...
    class Meta:
        ...
        managed = True
        indexes = [
            models.Index(fields=['patientId', 'imageType']),
            models.Index(fields=['status', 'createdAt']),
        ]
...
class DocumentAnalysis(AtomicBaseModel):
    ...
    documentUrl = models.URLField(verbose_name=_('Document URL'), db_column='document_url', null=True)
    ...
    class Meta:
        ...
        managed = True
        indexes = [
            models.Index(fields=['patientId', 'documentType']),
            models.Index(fields=['status', 'createdAt']),
        ]
